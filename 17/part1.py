...
def simulate_position(initial_x_speed, x_start, min_steps, y_range):
    x_pos = x_start
    y_speed = 0
    best_y_speed = y_speed
    best_x_speed = initial_x_speed
    while True:
        ...
        if y_speed > 1000:
            break
    return (best_x_speed, best_y_speed)
...
for x_step in possible_steps:
    ...
    min_steps = x_step[1]
    x_start = x_step[2]
    speed_tuple = simulate_position(x_speed, x_start, min_steps, y_range)
    if not best_speed_tuple or (speed_tuple[1] > best_speed_tuple[1]):
        best_speed_tuple = speed_tuple
best_y_speed = best_speed_tuple[1]
print(int((2*best_y_speed*best_y_speed - best_y_speed*best_y_speed + best_y_speed)/2))
